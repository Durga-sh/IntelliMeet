...
export class FFmpegService extends EventEmitter {
  ...
  private sdpDir: string;
  private ffmpegPath: string;

  constructor() {
    super();
    this.recordingsDir = join(process.cwd(), "recordings");
    this.sdpDir = join(process.cwd(), "sdp");
  ...
  private async ensureDirectories(): Promise<void> {
    ...
    try {
      await fs.access(this.sdpDir);
    } catch {
      await fs.mkdir(this.sdpDir, { recursive: true });
    }
  }
   ...
   */
  private createSDP(audioPort: number, videoPort: number): string {
    return `v=0
...
a=rtcp-fb:96 ccm fir`;
  ...
  async startRecording(options: FFmpegProcessOptions): Promise<string> {
    ...
    const sdpPath = join(this.sdpDir, `${roomId}.sdp`);
      ...
      command
        ...
        .on("progress", (progress) => {
          if (progress.timemark && progress.frames > 0) {
            console.log(
              `🎥 Recording ${roomId}: ${progress.timemark} (${progress.frames} frames)`
            );
          }
        ...
        .on("end", async () => {
          ...
          // Clean up SDP
          try {
            if (existsSync(sdpPath)) unlinkSync(sdpPath);
          } catch (err) {
          ...
          try {
            ...
            if (fileSize > 10000) {
              ...
              console.warn(
                `⚠️ Recording file is very small (${fileSize} bytes)`
              );
            }
